...
bool ByteArray::isLittleEndian() const { return m_endian == PHASE0_LITTLE_ENDIAN; }

void ByteArray::setIsLittleEndian(bool val)
{
    if (val)
    {
        m_endian = PHASE0_LITTLE_ENDIAN;
    }
    else
    {
        m_endian = PHASE0_BIG_ENDIAN;
...
bool ByteArray::writeToFile(const std::string& name) const
{
    ...
    if (!ofs)
    {
        P0SYS_LOG_ERROR() << "writeToFile name=" << name << " error , errno=" << errno
...
bool ByteArray::readFromFile(const std::string& name)
{
    ...
    if (!ifs)
    {
        P0SYS_LOG_ERROR() << "readFromFile name=" << name << " error, errno=" << errno
                                  << " errstr=" << strerror(errno);
        return false;
    }

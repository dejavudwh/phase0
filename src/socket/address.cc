    ...
    std::vector<Address::ptr>& result, const std::string& host, int family, int type, int protocol)
{
    ...
    if (error)
    {
        P0SYS_LOG_DEBUG() << "Address::Lookup getaddress(" << host << ", " << family << ", " << type
    ...
    while (next)
    {
        ...
        P0SYS_LOG_DEBUG() << "family:" << next->ai_family << ", sock type:" << next->ai_socktype;
        next = next->ai_next;
    }
                                    ...
                                    int family)
{
    ...
    {
        P0SYS_LOG_DEBUG() << "Address::GetInterfaceAddresses getifaddrs "
                             " err="
    ...
    {
        P0SYS_LOG_ERROR() << "Address::GetInterfaceAddresses exception";
        freeifaddrs(results);
        return false;
    }
    freeifaddrs(results);
...
IPAddress::ptr IPAddress::Create(const char* address, uint16_t port)
{
    ...
    if (error)
    {
        P0SYS_LOG_DEBUG() << "IPAddress::Create(" << address << ", " << port << ") error=" << error
...
IPv4Address::ptr IPv4Address::Create(const char* address, uint16_t port)
{
    ...
    if (result <= 0)
    {
        P0SYS_LOG_DEBUG() << "IPv4Address::Create(" << address << ", " << port << ") rt=" << result
                          << " errno=" << errno << " errstr=" << strerror(errno);
        return nullptr;
    }
    return rt;
...
IPv6Address::ptr IPv6Address::Create(const char* address, uint16_t port)
{
    ...
    if (result <= 0)
    {
        P0SYS_LOG_DEBUG() << "IPv6Address::Create(" << address << ", " << port << ") rt=" << result
                          << " errno=" << errno << " errstr=" << strerror(errno);
        return nullptr;
    }
    return rt;

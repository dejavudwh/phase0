...
namespace http
{
static phase0::ConfigVar<uint64_t>::ptr g_http_request_buffer_size =
    phase0::Config::lookup("http.request.buffer_size", (uint64_t)(4 * 1024), "http request buffer size");

static phase0::ConfigVar<uint64_t>::ptr g_http_request_max_body_size = phase0::Config::lookup(
    "http.request.max_body_size", (uint64_t)(64 * 1024 * 1024), "http request max body size");

static phase0::ConfigVar<uint64_t>::ptr g_http_response_buffer_size =
    phase0::Config::lookup("http.response.buffer_size", (uint64_t)(4 * 1024), "http response buffer size");

static phase0::ConfigVar<uint64_t>::ptr g_http_response_max_body_size = phase0::Config::lookup(
...
{
    P0SYS_LOG_DEBUG() << "on_request_message_begin_cb";
    return 0;
}

static int on_request_headers_complete_cb(http_parser* p)
{
    P0SYS_LOG_DEBUG() << "on_request_headers_complete_cb";
...
{
    P0SYS_LOG_DEBUG() << "on_request_message_complete_cb";
    HttpRequestParser* parser = static_cast<HttpRequestParser*>(p->data);
...
{
    P0SYS_LOG_DEBUG() << "on_request_chunk_header_cb";
    return 0;
}

static int on_request_chunk_complete_cb(http_parser* p)
{
    P0SYS_LOG_DEBUG() << "on_request_chunk_complete_cb";
    return 0;
}

static int on_request_url_cb(http_parser* p, const char* buf, size_t len)
{
    P0SYS_LOG_DEBUG() << "on_request_url_cb, url is:" << std::string(buf, len);
    ...
    if (ret != 0)
    {
        P0SYS_LOG_DEBUG() << "parse url fail";
        return 1;
    }
...
static int on_request_header_field_cb(http_parser* p, const char* buf, size_t len)
{
    ...
    P0SYS_LOG_DEBUG() << "on_request_header_field_cb, field is:" << field;
    HttpRequestParser* parser = static_cast<HttpRequestParser*>(p->data);
...
static int on_request_header_value_cb(http_parser* p, const char* buf, size_t len)
{
    ...
    P0SYS_LOG_DEBUG() << "on_request_header_value_cb, value is:" << value;
    HttpRequestParser* parser = static_cast<HttpRequestParser*>(p->data);
...
{
    P0SYS_LOG_DEBUG() << "on_request_status_cb, should not happen";
    return 0;
}
...
static int on_request_body_cb(http_parser* p, const char* buf, size_t len)
{
    ...
    P0SYS_LOG_DEBUG() << "on_request_body_cb, body is:" << body;
    HttpRequestParser* parser = static_cast<HttpRequestParser*>(p->data);
...
size_t HttpRequestParser::execute(char* data, size_t len)
{
    ...
    {
        //处理新协议，暂时不处理
        P0SYS_LOG_DEBUG() << "found upgrade, ignore";
        setError(HPE_UNKNOWN);
    }
    else if (m_parser.http_errno != 0)
    {
        P0SYS_LOG_DEBUG() << "parse request fail: " << http_errno_name(HTTP_PARSER_ERRNO(&m_parser));
...
{
    P0SYS_LOG_DEBUG() << "on_response_message_begin_cb";
    return 0;
}

static int on_response_headers_complete_cb(http_parser* p)
{
    P0SYS_LOG_DEBUG() << "on_response_headers_complete_cb";
...
{
    P0SYS_LOG_DEBUG() << "on_response_message_complete_cb";
    HttpResponseParser* parser = static_cast<HttpResponseParser*>(p->data);
    parser->setFinished(true);
    return 0;
}
...
{
    P0SYS_LOG_DEBUG() << "on_response_chunk_header_cb";
    return 0;
}

static int on_response_chunk_complete_cb(http_parser* p)
{
    P0SYS_LOG_DEBUG() << "on_response_chunk_complete_cb";
    return 0;
}

static int on_response_url_cb(http_parser* p, const char* buf, size_t len)
{
    P0SYS_LOG_DEBUG() << "on_response_url_cb, should not happen";
    return 0;
}
...
{
    std::string field(buf, len);
    P0SYS_LOG_DEBUG() << "on_response_header_field_cb, field is:" << field;
    HttpResponseParser* parser = static_cast<HttpResponseParser*>(p->data);
    parser->setField(field);
    return 0;
}
...
{
    std::string value(buf, len);
    P0SYS_LOG_DEBUG() << "on_response_header_value_cb, value is:" << value;
    HttpResponseParser* parser = static_cast<HttpResponseParser*>(p->data);
    parser->getData()->setHeader(parser->getField(), value);
    return 0;
}
...
{
    P0SYS_LOG_DEBUG() << "on_response_status_cb, status code is: " << p->status_code
...
{
    std::string body(buf, len);
    P0SYS_LOG_DEBUG() << "on_response_body_cb, body is:" << body;
    HttpResponseParser* parser = static_cast<HttpResponseParser*>(p->data);
    parser->getData()->appendBody(body);
    return 0;
}
...
size_t HttpResponseParser::execute(char* data, size_t len)
{
    ...
    {
        P0SYS_LOG_DEBUG() << "parse response fail: " << http_errno_name(HTTP_PARSER_ERRNO(&m_parser));
        setError((int8_t)m_parser.http_errno);
    }
    else
    {

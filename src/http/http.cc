    ...
    {                                                                                                       \
        ...
        {                                                                                                   \
            std::cout << "<key>:" << str.substr(last, key - last)                                           \
                      << " <decoded>:" << phase0::StringUtil::UrlDecode(str.substr(last, key - last))       \
                      << " <value>:" << str.substr(key + 1, pos - key - 1)                                  \
                      << " <decoded>:" << phase0::StringUtil::UrlDecode(str.substr(key + 1, pos - key - 1)) \
                      << std::endl;                                                                         \
        }                                                                                                   \
                                                                                                            \
        m.insert(std::make_pair(phase0::StringUtil::UrlDecode(trim(str.substr(last, key - last))),          \
                                phase0::StringUtil::UrlDecode(str.substr(key + 1, pos - key - 1))));        \
        if (pos == std::string::npos)                                                                       \
        {                                                                                                   \
            break;                                                                                          \
        }                                                                                                   \
...
void HttpRequest::initCookies()
{
    ...
    PARSE_PARAM(cookie, m_cookies, ';', phase0::StringUtil::Trim);
    m_parserParamFlag |= 0x4;
}
                             ...
                             bool secure)
{
    ...
    if (expired > 0)
    {
        ss << ";expires=" << phase0::Time2Str(expired, "%a, %d %b %Y %H:%M:%S") << " GMT";
    }
    if (!domain.empty())
    {
        ss << ";domain=" << domain;

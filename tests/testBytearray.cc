...
#define XX(type, len, write_fun, read_fun, base_len)                                        \
    {                                                                                       \
        ...
        phase0::ByteArray::ptr ba(new phase0::ByteArray(base_len));                           \
        for (auto& i : vec)                                                                 \
        {                                                                                   \
            ba->write_fun(i);                                                               \
        }                                                                                   \
        ...
        {                                                                                   \
            type v = ba->read_fun();                                                        \
            PHASE0_ASSERT(v == vec[i]);                                                     \
        }                                                                                   \
        PHASE0_ASSERT(ba->getReadSize() == 0);                                              \
        P0ROOT_LOG_INFO() << #write_fun "/" #read_fun " (" #type ") len=" << len    \
...
#define XX(type, len, write_fun, read_fun, base_len)                                        \
    {                                                                                       \
        ...
        P0ROOT_LOG_INFO() << #write_fun "/" #read_fun " (" #type ") len=" << len    \
                                  << " base_len=" << base_len << " size=" << ba->getSize(); \
        ba->setPosition(0);                                                                 \
        PHASE0_ASSERT(ba->writeToFile("/tmp/" #type "_" #len "-" #read_fun ".dat"));        \
        phase0::ByteArray::ptr ba2(new phase0::ByteArray(base_len * 2));                      \
        PHASE0_ASSERT(ba2->readFromFile("/tmp/" #type "_" #len "-" #read_fun ".dat"));      \
        ba2->setPosition(0);                                                                \
        PHASE0_ASSERT(ba->toString() == ba2->toString());                                   \
        PHASE0_ASSERT(ba->getPosition() == 0);                                              \
        PHASE0_ASSERT(ba2->getPosition() == 0);                                             \
    }
    XX(int8_t, 100, writeFint8, readFint8, 1);
    XX(uint8_t, 100, writeFuint8, readFuint8, 1);
    XX(int16_t, 100, writeFint16, readFint16, 1);
